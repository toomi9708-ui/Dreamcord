...
import { platformName } from "@dreamcordplugins/dreamcordHelper/utils";
...
import { isAnyPluginDev, isDreamcordGuild, isDreamcordSupport, isKnownIssuesCategory, isSupportChannel, tryOrElse } from "@utils/misc";
...
const TrustedRolesIds = [
    ...
    EQUICORD_TEAM, // Dreamcord Team
    DONOR_ROLE_ID, // Dreamcord Donor
    CONTRIB_ROLE_ID, // Dreamcord Contributor
...
async function generateDebugInfoMessage() {
    ...
    clientString += `${clientInfo.shortHash ? ` • [${clientInfo.shortHash}](<https://github.com/Dreamcord/Equibop/commit/${clientInfo.hash}>)` : ""}`;
    ...
    const commonIssues = {
        ...
        "Dreamcord DevBuild": !IS_STANDALONE,
...
function DevBuildConfirmModal(props: RenderModalProps) {
    ...
    return (
        ...
        >
            <div>
                <Paragraph>You are using a custom build of Dreamcord, which we do not provide support for!</Paragraph>

                <Paragraph className={Margins.top8}>
                    We only provide support for <Link href="https://dreamcord.org/download">official builds</Link>.
                    Either <Link href="https://dreamcord.org/download">switch to an official build</Link> or figure your issue out yourself.
                </Paragraph>
...
export default definePlugin({
    ...
    commands: [
        {
            name: "dreamcord-debug",
            description: "Send Dreamcord debug info",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isDreamcordGuild(ctx?.guild?.id, true),
            execute: async () => ({ content: await generateDebugInfoMessage() })
        },
        {
            name: "dreamcord-plugins",
            description: "Send Dreamcord plugin list",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isDreamcordGuild(ctx?.guild?.id, true),
            execute: async () => {
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (
                        ...
                        >
                            <div>
                                <Paragraph>You are using an outdated version of Dreamcord! Chances are, your issue is already fixed.</Paragraph>
                                <Paragraph className={Margins.top8}>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                openModal(props => (
                    ...
                    >
                        <div>
                            <Paragraph>You are using an externally updated Dreamcord version, which we do not provide support for!</Paragraph>
                            <Paragraph className={Margins.top8}>
                                Please either switch to an <Link href="https://dreamcord.org/download">officially supported version of Dreamcord</Link>, or
    ...
    renderMessageAccessory(props) {
        ...
        const dreamcordSupport = isDreamcordSupport(props.message.author.id);

        const shouldAddUpdateButton =
            !IS_UPDATER_DISABLED
            && ((isSupportChannel(props.channel.id) && dreamcordSupport))
        ...
        if (dreamcordSupport && isSupportChannel(props.channel.id) && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)) {
            if (props.message.content.includes("/dreamcord-debug") || props.message.content.includes("/dreamcord-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
                        variant="secondary"
                    ...
                    >
                        Run /dreamcord-plugins
                    </Button>
                );
            }
        }

        if (dreamcordSupport || (isSupportChannel(props.channel.id) || isKnownIssuesCategory(props.channel.parent_id))) {
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Dreamcord & Vencord plugin developers for support!
                <br />
