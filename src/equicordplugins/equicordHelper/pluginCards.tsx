...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of Dreamcord. Try updating!";
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    message.embeds?.forEach(embed => {
        if (!embed.url?.startsWith("https://dreamcord.org/plugins/") && !embed.url?.startsWith("https://vencord.dev/plugins/")) return;

        const isDreamcord = isDreamcordGuild(message.channel_id) && isDreamcordSupport(message.author.id);
        if (!isDreamcord) return;
    ...
    if (message.author.id === EQUIBOT_USER_ID && components?.length >= 4) {
        ...
        if (pluginUrl?.startsWith("https://dreamcord.org/plugins/") || pluginUrl?.startsWith("https://vencord.dev/plugins/")) {
