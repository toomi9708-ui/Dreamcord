...
import { ChromiumIcon, ClientIcon, DevBannerIcon, DiscordIcon, ElectronIcon, DreamcordIcon, names, settings } from ".";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const format = settings.store.format ?? "{devbannerIcon} {buildChannel} {buildNumber} ({buildHash}) | {dreamcordIcon} {dreamcordName} {dreamcordVersion} ({dreamcordHash})";
    ...
    const replaced = baseFormat
        ...
        .replace(/{dreamcordVersion}/g, VERSION)
        .replace(/{dreamcordHash}/g, gitHashShort)
        .replace(/{dreamcordPlatform}/g, getVersionInfo(false))
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{dreamcordIcon}":
                return <span key={`icon-dreamcord-${i}`} className="vc-discord-dev-banner-icons"><DreamcordIcon /></span>;
            case "{electronIcon}":
