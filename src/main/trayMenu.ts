...
function createDreamcordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Dreamcord",
            submenu: [
                {
                    label: "About Dreamcord",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update Dreamcord" : "Check for Updates",
                    click: () => sendToRenderer(IpcEvents.TRAY_CHECK_UPDATES)
                },
                {
                    label: "Repair Dreamcord",
...
export function patchTrayMenu(): void {
    ...
    Menu.buildFromTemplate = function (template: MenuItemConstructorOptions[]) {
        const alreadyPatched = template.some(item => item.label === "Dreamcord");
        if (isTrayMenu(template) && !alreadyPatched) {
            const insertIndex = findInsertIndex(template);
            const dreamcordItems = createDreamcordMenuItems();
            template.splice(insertIndex, 0, ...dreamcordItems);
        }
