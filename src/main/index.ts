...
if (!IS_VANILLA && !IS_EXTENSION) {
    app.whenReady().then(() => {
        ...
        protocol.handle("dreamcord", ({ url: unsafeUrl }) => {
            let url = decodeURI(unsafeUrl).slice("dreamcord://".length).replace(/\?v=\d+$/, "");
        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[Dreamcord] Installed React Developer Tools"))
                    .catch(err => console.error("[Dreamcord] Failed to install React Developer Tools", err));
        } catch { }
