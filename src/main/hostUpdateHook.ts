...
/**
 * re-applies the dreamcord patch to a freshly installed discord host
...
interface DiscordUpdaterModule {
    ...
    __dreamcordTryInitWrapped?: boolean;
}
...
interface DiscordDesktopCore {
    ...
    __dreamcordStartupWrapped?: boolean;
}

const error = (...args: unknown[]) => console.error("[Dreamcord:HostUpdate]", ...args);
...
const retainDreamcord = (updater: DiscordHostUpdater, reason: string) => {
    try {
...
const attachToUpdater = (updater: DiscordHostUpdater | null | undefined) => {
    ...
    updater.on?.("host-updated", () => retainDreamcord(updater, "host-updated"));
    ...
    if (typeof sync === "function") {
        ...
        updater.startCurrentVersionSync = (options?: StartCurrentVersionOptions) => {
            bound(options);
            try { retainDreamcord(updater, "startCurrentVersionSync"); } catch (e) { error(e); }
        };
    }
    ...
    if (typeof async_ === "function") {
        ...
        updater.startCurrentVersion = async (queryOptions?: object, options?: StartCurrentVersionOptions) => {
            ...
            try { retainDreamcord(updater, "startCurrentVersion"); } catch (e) { error(e); }
        };
    }
};

const wrapStartup = (coreExports: DiscordDesktopCore | null | undefined) => {
    if (!coreExports?.startup || coreExports.__dreamcordStartupWrapped) return;
    coreExports.__dreamcordStartupWrapped = true;
    ...
    coreExports.startup = function (opts, ...rest) {
        try {
            ...
            } else if (typeof updaterModule?.tryInitUpdater === "function" && !updaterModule.__dreamcordTryInitWrapped) {
                /*
                 * updater not yet constructed at startup time. wrap the
                 * factory so we attach once vanilla creates it.
                 */
                updaterModule.__dreamcordTryInitWrapped = true;
