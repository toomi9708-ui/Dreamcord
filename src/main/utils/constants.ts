...
export const DATA_DIR = process.env.EQUICORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "DreamcordData", suffix)
        : join(app.getPath("userData"), "..", "Dreamcord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Dreamcord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
