...
While it was primarily created for Dreamcord based on Vencord and its respective packages, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @dreamcord/discord-types
yarn add -D @dreamcord/discord-types
pnpm add -D @dreamcord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@dreamcord/discord-types";
...
```ts
import { ApplicationCommandType } from "@dreamcord/discord-types/enums";
